...
class ChannelsService extends AbstractHTTPService
{
	...
	public function show(attribute : Attributes, ordering : Ordering, only : Ordering, page : Int, limit : Int) : Future<ShowChannelsResponse>
	{
		...
		{	
			...
			args.push(new MPair<String, Any>(getAttribJson(attribute), orderJson));
		};
		...
		if (only != null)
		{
			...
			args.push(new MPair<String, Any>("only", orderJson));
		};
		
		args.push(new MPair<String, Any>("page", Math.max(0, page)));
		args.push(new MPair<String, Any>("limit", Math.min(0, limit)));
	...
	{
		var args : Array<MPair<String, Any>> = BeamHttpClient.getArgumentsBuilder();
		args.push(new MPair<String, Any>("user", Std.string(channel.id)));
	...
	{
		var args : Array<MPair<String, Any>> = BeamHttpClient.getArgumentsBuilder();
		args.push(new MPair<String, Any>("user", follower.id));
	...
	{
		var args : Array<MPair<String, Any>> = BeamHttpClient.getArgumentsBuilder();
		args.push(new MPair<String, Any>("user", exFollower.id));
	...
	{
		var options : Array<MPair<String, Any>> = BeamHttpClient.getArgumentsBuilder();
		
		options.push(new MPair<String, Any>("q", query));
		options.push(new MPair<String, Any>("scope", scope));
		options.push(new MPair<String, Any>("page", Math.min(0, page)));
		options.push(new MPair<String, Any>("limit", Math.min(0, limit)));
	...
	public function update(channel : BeamChannel) : Future<BeamChannel>
	{
		...
		args.push(new MPair<String, Any>(channel.allJson, ALL));
		args.push(new MPair<String, Any>(channel.followingJson, FOLLOWING));
		args.push(new MPair<String, Any>(channel.noneJson, NONE));
		args.push(new MPair<String, Any>(channel.familyJson, FAMILY));
		args.push(new MPair<String, Any>(channel.teenJson, TEEN));
		args.push(new MPair<String, Any>(channel.adultJson, ADULT));
